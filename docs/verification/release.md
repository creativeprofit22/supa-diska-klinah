...
**Unsigned release candidate: automated gates and non-elevated acceptance passed; elevated install/uninstall and the installer UAC/SmartScreen observations were accepted by the user on 2026-09-26, as were the per-operation UAC counts, the parity drivers on the installed build, the four build-artifact drill items and the Spanish review; the published update and the signed run remain open.** No release has been published from this candidate.
...
- [x] (Accepted by the user 2026-09-26.) The four open build-artifact drill items in the [release checklist](../release-checklist.md): budget enforcement/quarantine, protected identities, build-generation undo, screenshot set. Drill run on the installed build 2026-09-26 and repeated on the rebuilt installer carrying the protected-list layout fix (evidence below).

  Build-artifact drill on the installed build (2026-09-26, 16:13–16:23 UTC). The app was `C:\Program Files\Supa Diska Klinah\supa-diska-klinah.exe` (pid 14960), driven over the loopback test harness from a standard shell. The disposable fixture was `E:\sdk-budget-drill`, a copy of `windows-platform/tests/fixtures/build-artifacts/rust` plus a `build.rs` that sleeps 90 s only while `slow.gate` says `slow`. It was added as a project root (`86507adc…`). Artifacts: `.gg/smoke-artifacts/release-drill-budget/` (`steps.jsonl`, `identities-*.json` with SHA-256 per file, PNGs `01`–`05`). No UAC prompt is expected: the build-artifact path makes no helper calls.

  | Item | Result |
  | --- | --- |
  | Profiles | The user approved both native "Aprobar perfil de compilación" dialogs showing the absolute `cargo.exe`, argv, the working directory and the artifact paths. Release profile `26985760…` (`build --release --offline`; `target/release/artifact-budget-fixture.exe` as generation, `target/release/deps` as dependency). Debug profile `debd22cf…` (`build --offline`; the debug exe as generation, `deps` as dependency, `incremental` as incremental). Earlier attempts refused a PATH shim executable and a non-empty working directory with `invalidInput`, as designed |
  | Builds | Release run `6fa78657…` and debug run succeeded (exit 0); the release exe was owned and not the latest, and the debug outputs were the latest successful build |
  | Preview while disabled | Policy saved with `enabled: false` and a 3,000,000-byte global limit (`analysisStatus: notRequested`). The preview selected only `target/release/artifact-budget-fixture.exe` (124 KB). Everything else was protected with reasons: debug exe (current profile/target, latest success), debug `deps` and `incremental` (unowned, non-generation, dependency/incremental, current, latest success), release `deps` (unowned, non-generation, dependency). The protected floor was 3.6 MB above the 2.9 MB limit, and the UI said the limit cannot be met without deleting protected state |
  | Enforcement and quarantine | Enabling the same policy ran analysis (`completed`) and quarantined exactly the release exe: execution `abdd5d97…`, disposition `quarantine`, 126,976 bytes quarantined, 0 failed, `purgeAfter` 1791044514 (7 days). The preview then selected nothing |
  | Protected identities | Per-file SHA-256/size/mtime snapshots of the 109 files under `target\` before and after enforcement: 1 gone (the release exe), 0 added, 0 changed, 108 byte-identical including every `deps`, `incremental` and debug file |
  | Undo | `undo_cleanup` on `abdd5d97…` restored the item (`state: restored`, 0 failed). All 109 files present again; the release exe has the same SHA-256 (`0E075AE6…F67B6`) and size, with only its mtime renewed by the restore |
  | Running/cancelled | The policy was set back to disabled with no limits before this step. Clicking the debug profile's Ejecutar showed "Última compilación: en ejecución" with a Cancelar button. Clicking Cancelar ended run `a5492ae6…` as `cancelled` in about 14 s, with "Última compilación: cancelada"; no cargo/rustc/build-script process remained |
  | Screenshots | `01-disabled.png` (settings, apply budgets unchecked), `02-selected-preview.png`, `03-protected-floor.png`, `04-running.png`, `05-cancelled.png`. `03` shows a cosmetic defect in that build: in the protected-generations list, long path names overlapped the reason text. It was fixed after the drill by putting each reason list on its own row and wrapping long paths. The fix was checked in a static page that uses the real stylesheet and the drill's Spanish strings, at 700 px and 360 px wide (`.gg/ui-check/wide.png`, `narrow.png`). The build-artifacts tests (53) and `pnpm check` pass. The installed candidate predates the fix, so `03` has not been retaken on a rebuilt installer |
  | Retake on rebuilt installer | The installer was rebuilt with the fix at HEAD `a69db27` plus the uncommitted list fix (command `07c7bad2-3d66-475c-9ebb-dd09ad2c8620`): 3,577,473 bytes, SHA-256 `8AB597A8E0C4369B0C1BC2A7211DF0A83CCE324732326B29046068370516E1D3`, `NotSigned`. Silent per-machine reinstall exited 0 with one UAC prompt (command `e921db0f-93d6-421f-9bc7-70a345a38236`); installed app SHA-256 `E5063FCA…BCDE`, helper present. Fixture recreated at `E:\sdk-budget-drill`, app pid 5952, root `1e7efb14…` (artifacts `.gg/smoke-artifacts/release-drill-budget-retake/`). The user approved both native dialogs: release profile `12446f75…` (run `2f097752-bdde-49be-9834-ee00b9077a12`, finished 16:43:16 UTC) and debug profile `21cc9393…` (run `1cda6eaf-50aa-4b83-a0d3-50a2f95ef412`). Builds `97752a96…` (release) and `e4764dbc…` (debug) both succeeded with exit 0. The disabled-policy preview again selected only the release exe, with 4 protected entries and a 3,109,728-byte protected floor. The retaken `.gg/smoke-artifacts/release-drill-budget-retake/03-protected-floor.png` shows each path on its own line above its reasons, with no overlap. Enabling the policy quarantined only the release exe (execution `b84b5566…`, `purgeAfter` 1791045896): 38 files before and 1 gone after, with the rest byte- and mtime-identical. Undo restored it: 38 files, all SHA-256 equal to before. Teardown (command `bf0defa3-21a0-4c02-a58f-d7ebfd02e95d`): profiles and root removed, policy disabled with no limits, no active run, the app closed, `E:\sdk-budget-drill` deleted. Driver commands `0e888ff3-f0a3-47f9-a8de-c52a851a6506` (builds and screenshot) and `523f54e8-ed3f-4fb4-87d2-43814fd5c339` (enforce and undo). No UAC prompt beyond the reinstall |
